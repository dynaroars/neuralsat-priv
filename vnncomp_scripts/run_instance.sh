...
if [ -f $DNNV_ONNX_SIM ]; then
	...
	$NEURALSAT_PY $NEURALSAT_MAIN --net $DNNV_ONNX_SIM --spec $VNNLIB_FILE --timeout $TIMEOUT --verbosity=2 --result_file $RESULTS_FILE
...
if [ -f $ONNXSIM_ONNX_SIM ]; then
	...
	# success
	$NEURALSAT_PY $NEURALSAT_MAIN --net $ONNXSIM_ONNX_SIM --spec $VNNLIB_FILE --timeout $TIMEOUT --verbosity=2 --result_file $RESULTS_FILE
